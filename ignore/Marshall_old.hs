...
cvtStr s = Str s
-- xmlToVal :: Content i -> Val
-- xmlToVal (CString _ cdata _) = cvtStr cdata
-- xmlToVal (CElem elem _) =
--     let Elem qname _ cs = elem in
--     let Text.XML.HaXml.Types.N n = qname in
--     Nod (cvtStr n) (foldr (:@) Nl (map xmlToVal cs))
-- cvtStr :: String -> Val
-- cvtStr "undefined" = Undef
-- cvtStr s = Str s
